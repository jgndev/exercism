...
namespace estate_executor {
    int assemble_account_number(int secret_modifier) {
        const int z = zhang::bank_number_part(secret_modifier);
        const int k = khan::bank_number_part(secret_modifier);
        const int g = garcia::bank_number_part(secret_modifier);

        return z + k + g;
    }

    int assemble_code() {
        const int red = zhang::red::code_fragment() +
                        khan::red::code_fragment() +
                        garcia::red::code_fragment();
        const int blue = zhang::blue::code_fragment() +
